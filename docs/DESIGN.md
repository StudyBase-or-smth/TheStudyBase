...
| Version | 7.2.0 |
| Badge date | 23-8-26 |
| Source of truth | Live files on this branch |
...
2. **Shared chrome, local accent.** Every signed-in page uses the same header: left accent bar, wordmark or breadcrumb, ghost-button actions, notification bell, profile. The accent colour is amber on the hub and the subject’s own colour on subject/class/flashcard pages.
...
4. **Fail closed.** Missing or empty approval status is treated as pending. Token refresh failures send the user to the waiting page rather than granting hub access.
...
Self-hosted variable fonts (so ad-blockers cannot strip Google Fonts):
...
Base size is 14px. The hub content column is **1060px** max, padded 24px.
...
Subject pages set `--accent` from the subject’s catalogue colour. Dev Panel uses purple `#7c3aed`. The Analyser uses its own dark amber palette and is not bound to hub tokens.

### Recurring UI patterns

- **Accent bar** — 6px colour strip on the left of the header. On hub, a developer’s bar grows to 36px on hover and opens the Dev Panel. On inner pages the same strip is a back control (`.is-back`).
- **Ghost buttons** — bordered, 5px radius, `data-tip` hover labels. Used for sync, suggestions, notifications, profile.
...
Sign-up offers **student** or **teacher** only. Dev accounts are created by an existing developer.
...
| **Pending / rejected** | Held on `waiting.html`. Can continue as guest (after sign-out). |
...
4. Every protected page **force-refreshes** the ID token so a newly approved (or rejected) claim is visible immediately.
...
| Branch badge | `Backend · v7.2.0` from `BranchVersion.js` / `.json`. |
| Sync | Manual pull; countdown to the next automatic sync (~60s on hub and subject pages). |
| Suggestions | Modal for bugs, requests, and ideas. Shared list, filterable Open / Closed / All. |
| Bell | Site notifications and polls. Badge shows unread count (`9+` max). |
...
Full-height workspace: 268px sidebar + detail pane.

- Sidebar: search, unit filter, **Subject / Classes** tabs, topic list (pin, expand subtopics).
...
- Hero: avatar, name, email, member since, role badge.
...
Separate dark tool for **AI assignment marking**.
...
Practice quizzes loaded from `HSC/storage/` via a manifest. Sidebar picks a paper and lists questions with mark deltas. Multiple choice and math fills grade locally; open responses can be sent to `/api/grade` when the user is active. Summary screen shows marks and a path to the next section.

### Login / waiting

Login is an `auth-page` card: Sign In / Sign Up tabs, dark toggle, guest link. Waiting has three states — checking, pending, rejected — with “Check again” and “Continue as Guest”.

### Dev Panel — `Devpanel.html`

Purple, compact, developer-only.

- Account table: search, status segments, approve / deny / deactivate / delete / rename / change role.
- Site stats drawer (topic/unit counts from cached subject data).
- Console for notification commands (`notify`, `notify-ask`, `notify-remove`).
- Bootstrap-role tool for assigning a role to a UID.
- Tag click opens a hidden Tetris page (`devbackend.html`) — not part of the product surface.
...
- The hub and subject pages pull about every 60 seconds. Notifications pull about every 45 seconds and on tab focus.
...
| `/api/rejectUser` | Dev | Reject or delete |
...
Site-wide bell on the hub, subject, and class pages. Store key: `studybase_notifications`.

| Kind | Behaviour |
| --- | --- |
| `text` | Broadcast. Marked read when the recipient opens the bell. |
| `ask` | Poll, 2–8 options. “Read” means answered. Finished asks collapse to a dropdown. Devs receive an automatic results summary. |

Targets from the Dev Panel: everyone, a role, or a single user (UID, email, or display name). Fully consumed notifications are pruned from the store.
