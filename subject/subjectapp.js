  ...
  document.getElementById('topicList').innerHTML = topLevel.length === 0
    ...
    : topLevel.map(t => {
        ...
        <div class="topic-item-wrap">
          <div class="topic-item${(t.id==activeId && !activeOrigin)?' active':''}${isPinned?' pinned':''}" onclick="viewTopic(${jsArg(t.id)})" oncontextmenu="return topicContextMenu(event, ${jsArg(t.id)})">
            <div class="ti-top">
              <div class="ti-name">
...
function renderClassNode(t, topics, cls){
  ...
  return `
    <div class="topic-item-wrap">
      <div class="topic-item${isActive ? ' active' : ''}" onclick="viewTopic(${jsArg(t.id)}, ${originArg})" oncontextmenu="return topicContextMenu(event, ${jsArg(t.id)})">
        <div class="ti-top">
          <div class="ti-name">
...
function viewTopic(id, originId){
  ...
  el.innerHTML = `
      <div class="dh">
        <div>
          <div class="dh-name${titleUnsynced ? ' unsynced-title' : ''}"${titleUnsynced ? ' title="unable to connect to servers"' : ''} oncontextmenu="return topicContextMenu(event, ${jsArg(t.id)})">${esc(t.name)}</div>
          <div class="dh-meta">
            ${originBadge}
        ...
        <div class="dh-actions">
          ${(window.isGuest || (origin && window.userRole !== 'dev')) ? '' : `<div class="dh-action-btns"><button class="btn-act" onclick="openModal(${jsArg(t.id)})">Edit</button>
...
function closeModal(){ document.getElementById('modalOverlay').classList.remove('open'); editId = null; editOrigin = null; destroyDesmosEditor(); _modalSnapshot = null; }
